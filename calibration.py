...
def _guess_element_symbol_from_name(name: str):
    ...
    low = base.lower()
    # common embedded token patterns: FeKa, Al_K, Fe-map, FeKa_map etc.
...
class ImageTextViewer(QWidget):
    ...
    def on_motion(self, event):
        if event.inaxes and self.start_point is not None:  # Only draw if mouse is pressed
...
class CalibrationGUI(QWidget):
    ...
    def show_compute_panel(self):
        ...
        for file_path in text_files:
            ...
            _sym = _guess_element_symbol_from_name(file_name)
            column_name = f"{_sym} (intensity)" if _sym else f"{file_name} (intensity)"
...
if __name__ == "__main__":
    ...
    gui = CalibrationGUI()
    gui.setWindowTitle("Calibration")
    gui.show()
    sys.exit(app.exec_())
